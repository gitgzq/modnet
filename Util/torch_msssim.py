...
class MS_SSIM(torch.nn.Module):
    ...
    def _ssim(self, img1, img2, size_average=False):
        ...
        if size_average:
            ...
            b = ssim_map.size()[0]
            ssim_map = torch.mean(torch.reshape(ssim_map,(b,-1)),dim=1,keepdim=False).cuda()
            mcs_map = torch.mean(torch.reshape(mcs_map,(b,-1)),dim=1,keepdim=False).cuda()
    ...
    def ms_ssim(self, img1, img2, levels=5):
        ...
        b = img1.size()[0]
        msssim=Variable(torch.Tensor(b,levels))
        mcs=Variable(torch.Tensor(b,levels))
        ...
        value=(torch.prod(mcs[:,:levels-1]**weight[:levels-1],dim=1) *
